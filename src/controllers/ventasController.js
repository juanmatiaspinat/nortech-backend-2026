...
exports.createVenta = async (req, res, next) => {
  try {
    ...
    //BUSCA usuario local por UUID Supabase
    const usuario = await getUsuarioByAuthId(auth_id);

    if (!usuario) {
      return res.status(404).json({
        error: "Usuario no encontrado en base local.",
      });
    }
    ...
    if (!productos || productos.length === 0) {
      return res.status(400).json({
        error: "Debes incluir productos en la venta.",
      });
    }
    ...
    console.log("ERROR CREATE VENTA:", error);
